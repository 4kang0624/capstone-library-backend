...
from app.schemas.wallet import WalletNonceRequest, WalletNonceResponse, WalletResponse, WalletVerifyRequest
...
def issue_wallet_nonce(
	...
	"""지갑 검증용 nonce 발급"""
	nonce, message = WalletService.issue_nonce(db, current_user.id, request.wallet_address)
...
def verify_wallet_signature(
	...
	result = WalletService.verify_signature_and_save(
		db, current_user.id, request.wallet_address, request.nonce, request.signature
	)
	if not result:
...
def get_my_wallet(
	...
	return WalletResponse(wallet_address=current_user.wallet_address)
